...
|---|---|---|
| Durasi Sempro = 60 menit | 44/45 baris | sesuai asumsi |
| Durasi Semhas = **105 menit** | 46/48 baris (bukan 120) | slot generator pakai 105 |
...
| Jadwal kuliah pakai sistem **Sesi 1–4** | jam tergantung SKS | Edge Function ekspansi Sesi → jam sebelum kirim ke GA |
...
| Sesi | 2 SKS | 3 SKS |
|---|---|---|
| 1 | 07.30–09.10 | 07.30–10.00 |
| 2 | 09.20–11.00 / 10.20–12.00 ⚠️ | — |
| 3 | — | 13.00–15.30 |
| 4 | 16.00–17.40 | — |
...
`chromosome = [slot_idx_1, …, slot_idx_N]`, panjang N = jumlah seminar dalam batch. Posisi ke-`i` = identitas seminar tetap. Nilai = index ke `domain[i]` (slot kandidat yang lolos H3–H5). Crossover & mutation gene-wise.
...
`POST {AI_URL}/solve` — detail request/response di [`api-contract.md`](api-contract.md) §E. Payload sudah bersih (jam-range + `dosen_id`); GA tidak tahu aturan SKS.

## 10. Isu Terbuka

> Terjawab (2026-09-10): venue `online` **ditetapkan admin** per seminar (`seminars[].is_online`), GA hanya menjadwalkan waktunya. Sisa:

1. ⚠️ Sesi 2 dua varian jam — mana yang berlaku, atau ambil rentang terluas (09.20–12.00)?
...
Testing: unit test per constraint (kasus sengaja bentrok → terdeteksi; domain kosong → `unscheduled`, bukan crash), lalu **benchmark vs jadwal manual Genap 25/26**:
- `conflict_count` (H1+H2) **wajib 0** di setiap hasil.
- S0 ditekan serendah mungkin (baseline manual: 14 sempro, 18 semhas).
- S1 jauh di bawah baseline 41/126.
